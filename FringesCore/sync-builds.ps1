...
Write-Host "FringesCore Build System Sync" -ForegroundColor Cyan
...
# Extract FRINGES_SOURCES - Match everything until we find the closing parenthesis at start of line
# This handles comments with parentheses inside the set() block
$sourcesMatch = [regex]::Match($cmakeFile, 'set\(FRINGES_SOURCES\s+(.*?)\n\)', [System.Text.RegularExpressions.RegexOptions]::Singleline)

if (-not $sourcesMatch.Success) {
    Write-Host "Error: Could not find FRINGES_SOURCES in CMakeLists.txt" -ForegroundColor Red
    exit 1
}
...
function Get-ExecutablesFromCMake {
    ...
    foreach ($match in $matches) {
        ...
        foreach ($line in $filesBlock -split "`n") {
            ...
            if ($line -match '([\w/]+\.cpp)') {
                $file = $matches[1]
                # Make path relative to FringesCore root
...
$allHeaders = Get-ChildItem -Path "include\FringesCore" -Recurse -File |
...
# Read .vcxproj
$vcxprojFile = "FringesCore.vcxproj"
...
$filtersFile = "FringesCore.vcxproj.filters"
if (Test-Path $filtersFile) {
    ...
    if ($filtersCompileGroup -and $filtersIncludeGroup) {
        ...
        function Get-FilterPath {
            ...
            elseif ($filePath -match '^include\\(\w+)\\') {
