...
namespace
{
	...
	// Helper function for bilinear solver
	...
	inline void performBilinearInterpolation(
	...
	// Helper function for HorizontalLinear solver
...
namespace {
	...
	void fillZk(const Interpolator& interpolator,
		...
		std::vector<double>& zk)
	{
		for (int row = 0; row < outHeight; ++row)
		{
			...
			const std::size_t rowOffset =
				static_cast<std::size_t>(row) * static_cast<std::size_t>(outWidth);
	...
	const WavefrontFromContoursContext& ctx) const
{
	...
	switch (method_)
	{
	case InterpolationMethod::LocalQuadric:
		fillZk(LocalQuadricInterpolator(std::move(samples)),
			ctx, mask, outWidth, outHeight, zk);
		break;

	case InterpolationMethod::DeCasteljau:
		fillZk(DeCasteljauInterpolator(std::move(samples)),
			ctx, mask, outWidth, outHeight, zk);
		break;

	case InterpolationMethod::AkimaBivariate:
		fillZk(AkimaBivariateInterpolator(std::move(samples)),
			ctx, mask, outWidth, outHeight, zk);
		break;

	case InterpolationMethod::Barycentric:
		fillZk(BarycentricInterpolator(std::move(samples)),
			ctx, mask, outWidth, outHeight, zk);
		break;

	case InterpolationMethod::IDW:
		fillZk(IDWInterpolator(std::move(samples)),
			ctx, mask, outWidth, outHeight, zk);
		break;
	}
