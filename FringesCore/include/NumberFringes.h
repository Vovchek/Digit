/**
 * @file NumberFringes.h
 * @brief Header-only classes for autoassignment of numbers to isoline fringes
...
inline std::vector<size_t> autoNumberFringes(
    ...
    const std::vector<size_t>& trustedFringeIndices,
    double step,
    [[maybe_unused]] double confidenceThreshold
) {
...
namespace impl_saddles {
    ...
    struct FringeNode {
        ...
        aperture::Bounds boundingBox;            ///< Bounding box of fringes
     ...
     */
    inline bool IsFringeClosed(const FringeSegment& fringe, double closureTol = 5.0)
    {
        int count = fringe.getPointCount();
        if (count < 3) return false;
        aperture::Point first = fringe.getPoint(0);
        aperture::Point last = fringe.getPoint(count - 1);
     ...
     */
    inline void ComputeCentroid(const FringeSegment& fringe, double& cx, double& cy)
    {
        int count = fringe.getPointCount();
        cx = 0.0;
        cy = 0.0;
        ...
        for (int i = 0; i < count; ++i) {
            aperture::Point pt = fringe.getPoint(i);
            cx += pt.x;
            cy += pt.y;
        }
        cx /= count;
     ...
     */
    inline void ComputeBoundingBox(const FringeSegment& fringe, aperture::Bounds& box)
    {
        int count = fringe.getPointCount();
     ...
     */
    inline bool IsPointInsidePolygon(const FringeSegment& poly, double x, double y)
    {
        int n = poly.getPointCount();
        if (n < 3) return false;
        bool inside = false;
        for (int i = 0, j = n - 1; i < n; j = i++) {
            aperture::Point pi = poly.getPoint(i);
            aperture::Point pj = poly.getPoint(j);
     ...
     */
    inline bool IsFullyInside(const FringeSegment& inner, const FringeSegment& outer)
    {
        if (!IsFringeClosed(outer)) return false;
        int count = inner.getPointCount();
        for (int i = 0; i < count; ++i) {
            aperture::Point pt = inner.getPoint(i);
    ...
    inline bool IsFullyInside(
        ...
        const std::vector<FringeSegment>& fringes)
    {
        ...
        for (auto idxOut : outer.indices) {
            ...
            aperture::Point pt = fringes[inner.primaryIndex].getPoint(0);
        ...
        const std::vector<FringeSegment>& fringes)
    {
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
            bool isEnclosedInRing = (nodes[i].outerNodeIdx != SIZE_MAX);
            
            if (isEnclosedInRing || nodes[i].isClosed) {
                // Part of ring structure
                result.ringNodeIndices.push_back(i);
            } else {
                // Topmost band (open and not enclosed)
                result.bandNodeIndices.push_back(i);
            }
        }
    ...
    struct HelperNode {
        const FringeSegment& fringe_;
        size_t i_;
        aperture::Bounds bounds_;
        double centroidX_;
        double centroidY_;
        HelperNode(const FringeSegment& fr, size_t i) :fringe_(fr), i_(i) {
            ComputeBoundingBox(fringe_, bounds_);
            ComputeCentroid(fringe_, centroidX_, centroidY_);
        };
        bool isSeparateFrom(const HelperNode& other, double proximityThreshold = 0.0) const {
            return
                (bounds_.left > other.bounds_.right + proximityThreshold) ||
                (other.bounds_.left > bounds_.right + proximityThreshold) ||
                (bounds_.top > other.bounds_.bottom + proximityThreshold) ||
                (other.bounds_.top > bounds_.bottom + proximityThreshold);
        }
    };

    struct GroupMetadata {
        aperture::Bounds boundingBox;
        double centroidX;
        double centroidY;
        size_t primaryIndex;
        bool isTrusted;
    ...
    inline Orientation orientation(const aperture::Point& p, const aperture::Point& q, const aperture::Point& r) {
    ...
    inline bool onSegment(const aperture::Point& p, const aperture::Point& r, const aperture::Point& q, double tolerance = 1e-9) {
    ...
    inline bool segmentsIntersect(const aperture::Point& p1, const aperture::Point& p2,
        const aperture::Point& p3, const aperture::Point& p4,
    ...
    inline bool checkPointsProximity(const aperture::Point& p1, const aperture::Point& p2, double distance) {
        if (distance <= 0.0) {
    ...
    inline bool pointsAreClose(const aperture::Point& p1, const aperture::Point& p2, double distance) {
        return checkPointsProximity(p1, p2, distance);
    }

    // Функция для проверки близости точки к отрезку
    inline bool pointNearSegment(const aperture::Point& point, const aperture::Point& segStart,
        const aperture::Point& segEnd, double distance) {
        if (distance <= 0.0) {
    ...
    {
        size_t countA = a.fringe_.getPointCount();
        size_t countB = b.fringe_.getPointCount();
        ...
        if (isPoint1 && isPoint2) {
            return checkPointsProximity(a.fringe_.getPoint(0), b.fringe_.getPoint(0), proximityThreshold);
        }

        if (proximityThreshold > 0.0) {
            // Check if any points are within proximity threshold 
            for (size_t i = 0; i < countA; ++i) {
                const aperture::Point& pa = a.fringe_.getPoint(i);
                for (size_t j = 0; j < countB; ++j) {
                    const aperture::Point& pb = b.fringe_.getPoint(j);
            ...
            for (size_t i = 0; i < countA; ++i) {
                const aperture::Point& point = a.fringe_.getPoint(i);
                for (size_t j = 0; j < countB - 1; ++j) {
                    const aperture::Point& pb1 = b.fringe_.getPoint(j);
                    const aperture::Point& pb2 = b.fringe_.getPoint(j + 1);
            ...
            for (size_t i = 0; i < countB; ++i) {
                const aperture::Point& point = b.fringe_.getPoint(i);
                for (size_t j = 0; j < countA - 1; ++j) {
                    const aperture::Point& pa1 = a.fringe_.getPoint(j);
                    const aperture::Point& pa2 = a.fringe_.getPoint(j + 1);
        ...
        if (!isPoint1 && !isPoint2) { // Только если обе полилинии имеют отрезки
            for (size_t i = 0; i < countA - 1; ++i) {
                const aperture::Point& pa1 = a.fringe_.getPoint(i);
                const aperture::Point& pa2 = a.fringe_.getPoint(i + 1);
                for (size_t j = 0; j < countB - 1; ++j) {
                    const aperture::Point& pb1 = b.fringe_.getPoint(j);
                    const aperture::Point& pb2 = b.fringe_.getPoint(j + 1);
        ...
        if (isPoint1) {
            ...
            const aperture::Point& point = a.fringe_.getPoint(0);
            for (size_t j = 0; j < countB - 1; ++j) {
                const aperture::Point& pb1 = b.fringe_.getPoint(j);
                const aperture::Point& pb2 = b.fringe_.getPoint(j + 1);
                if (pointNearSegment(point, pb1, pb2, proximityThreshold)) {
        ...
        if (isPoint2) {
            ...
            const aperture::Point& point = b.fringe_.getPoint(0);
            for (size_t i = 0; i < countA - 1; ++i) {
                const aperture::Point& pa1 = a.fringe_.getPoint(i);
                const aperture::Point& pa2 = a.fringe_.getPoint(i + 1);
                if (pointNearSegment(point, pa1, pa2, proximityThreshold)) {
    ...
    inline std::vector<GroupMetadata> MergeConnected(const std::vector<FringeSegment>& fringes,
        const std::map<size_t, bool>& trustedMap, double step)
    {
        ...
        for (auto& group : mergeGroups) {
            aperture::Bounds mergedBox = { 1e10, 1e10, -1e10, -1e10 };
            double sumX = 0.0, sumY = 0.0;
            bool hasTrusted = false;
            double k = 0.0;
            size_t primaryIndex = group[0].i_;
            std::vector<size_t> indices;
            for (const auto& node : group) {
                mergedBox.left = (std::min)(mergedBox.left, node.bounds_.left);
                mergedBox.right = (std::max)(mergedBox.right, node.bounds_.right);
                mergedBox.top = (std::min)(mergedBox.top, node.bounds_.top);
                mergedBox.bottom = (std::max)(mergedBox.bottom, node.bounds_.bottom);
                sumX += node.centroidX_;
                sumY += node.centroidY_;
                bool isNodeTrusted = (trustedMap.count(node.i_) > 0);
                if (isNodeTrusted) {
                    k = std::round(node.fringe_.getNumber() / step);
                    primaryIndex = node.i_; // Override primary index to trusted fringe
                }
                hasTrusted = hasTrusted || isNodeTrusted;
                indices.push_back(node.i_);
            }
        ...
        double step)
    {
        ...
        aperture::Point alignmentDir = { 1.0, 1.0 };
