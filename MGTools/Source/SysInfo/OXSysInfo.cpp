...
BOOL COXSysInfo::GetUserAndDomainName(CString *psUserName, CString *psDomainName) const
{
		...
		// Not Win NT
	{
		...
		*psUserName = _T("<UNKNOWN>");
		*psDomainName = _T("<UNKNOWN>");
	}
								  ...
								  int *pnDiskType) const
{
	...
	_stprintf(szRoot, _T("%c:\\"), 65 + nDrive);
									...
									DWORDLONG *pdwFreeSpace) const
{
	...
	*pdwVolSer = dwVolSer;
	...
	if(IsOSR2(&bIsOSR2) && bIsOSR2)
	{
		...
		if (hLib != NULL)
		{
			...
			GETDISKFREESPACEEX	dGetDiskFreeSpaceEx = 
				(GETDISKFREESPACEEX) GetProcAddress(hLib, "GetDiskFreeSpaceEx");
			...
			if (dGetDiskFreeSpaceEx != NULL)
			{
					...
					&ulTotalFree) > 0)
				{
					...
					*pdwTotalSpace = (DWORDLONG) ulTotalBytes.LowPart;

					// Get free disk space...
					*pdwFreeSpace = (DWORDLONG) ulTotalFree.LowPart;
				}
			}
			else
			{
	...
	{
		if (GetDiskFreeSpace(szRoot, &dwSectorsPerCluster, &dwBytesPerSector, &dwNumberOfFreeClusters, &dwTotalNumberOfClusters) == 0)
			return (FALSE);

		// Get total disk space...
		*pdwTotalSpace = (DWORDLONG)dwSectorsPerCluster * (DWORDLONG)dwBytesPerSector * 
			(DWORDLONG)dwTotalNumberOfClusters;
			
		// Get free disk space...
		*pdwFreeSpace = (DWORDLONG)dwSectorsPerCluster * (DWORDLONG)dwBytesPerSector * 
			(DWORDLONG)dwNumberOfFreeClusters;
	}
...
BOOL COXSysInfo::GetDisplayNumColors(DWORDLONG *pdwNumColors, int *pnNumBits) const
{
	...
	int		iBitsPixel = DeviceCaps(BITSPIXEL);
	int		i;
	*pdwNumColors = 1;
...
BOOL COXSysInfo::GetTotalPhysicalMemory(DWORD *pdwPhysMem)
{
	//	--- In:			
...
BOOL COXSysInfo::GetFreePhysicalMemory(DWORD *pdwFreeMem)
{
	//	--- In:			
...
BOOL COXSysInfo::GetTotalPageFile(DWORD *pdwTotalPageFile)
{
	//	--- In:			
...
BOOL COXSysInfo::GetFreePageFile(DWORD *pdwFreePageFile)
{
	//	--- In:			
...
BOOL COXSysInfo::GetTotalVirtual(DWORD *pdwTotalVirtual)
{
	//	--- In:			
...
BOOL COXSysInfo::GetFreeVirtual(DWORD *pdwFreeVirtual)
{
	//	--- In:			
...
BOOL COXSysInfo::GetProcessorType(DWORD *pdwProcessorType)
{
	...
	*pdwProcessorType = (DWORD)PROCESSOR_UNKNOWN;
		...
		*pdwProcessorType == (DWORD)PROCESSOR_UNKNOWN)
	{
		...
		// Pentium II
		case 6:
			*pdwProcessorType=PROCESSOR_INTEL_PENTIUM2;
			break;
		default:
			*pdwProcessorType=(DWORD)PROCESSOR_UNKNOWN;
			break;
...
{ 
	...
	*pdwProcessorSpeed=0;
	...
	{
		nNewMargin=abs(long(dwSpeed-arrCPUSpeed[nIterator]));
...
BOOL COXSysInfo::GetModemInfo(CArrayModemInfo* parrModemInfo) const
{
	...
	if(bSuccess)
	{
			...
			(_MAX_PATH+1)*sizeof(TCHAR))==ERROR_SUCCESS)
		{
			...
			if(::RegOpenKeyEx(hSubKey,sModemSubKeyPath,0,KEY_QUERY_VALUE,&hModemSubKey)==
				ERROR_SUCCESS)
			{
...
BOOL COXSysInfo::GetNumModems(int *pnNumModems) const
{
	...
	{
		*pnNumModems=arrModemInfo.GetSize();
		return TRUE;
	}
}
...
BOOL COXSysInfo::GetNICAddress(LPNICADDRESS pNICAddress) const
{
	...
	if(::CoCreateGuid(&guid)!=S_OK)
		...
		for(int nIndex=0; nIndex<countof(pNICAddress->data); nIndex++)
